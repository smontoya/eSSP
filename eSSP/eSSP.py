...
#       return result
    ...
    def poll(self):
        ...
        for i in range(3, int(result[2], 16) + 3):
            # if result[i] == '0xef' or result[i] == '0xee' or result[i] ==
            # '0xe6' or result[i] == '0xe1' or result[i] == '0xe2':
    ...
    # SPP_CMD_MANUFACTURER 0x30 not implented -
    # collides with SSP_CMD_EXPANSION ?!

    # SSP_CMD_EXPANSION 0x30 not implented -
    # collides with SSP_CMD_MANUFACTURER ?!
...
#                   self._logger.debug( " 8 || " + hex(crc) )
    ...
    def send(self, command, no_process=0):
        ...
        self._logger.debug("OUT: 0x" + ' 0x'.join([prepedstring[x:x+2] for x in range(0, len(prepedstring), 2)]))
        # prepedstring = binascii.b2a_uu(prepedstring)
        # prepedstring = prepedstring.decode('hex')
        prepedstring = codecs.decode(prepedstring, 'hex')
    ...
    def read(self, no_process=0):
        ...
        response += self.__ser.read(response[2] + 2)
    ...
    def process_response(self, response):
        ...
        if response[0] == '0x7f':
            ...
            if (response[-2:-1] != crc[0]) & (response[-1] != crc[1]):
